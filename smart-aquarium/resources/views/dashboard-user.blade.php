...
<div class="main-content">
    <!-- Top Navbar -->
    <nav class="navbar navbar-expand-lg navbar-dark shadow-lg rounded-bottom" style="background: radial-gradient(circle, rgba(13,110,253,0.15) 0%, rgba(102,16,242,0.15) 80%); backdrop-filter: blur(14px); border-radius: 1rem;">
        <div class="container-fluid p-3">
            <a class="navbar-brand fw-bold text-white" href="#">
                <i class="fas fa-fish me-2"></i> Smart Aquarium
            </a>
            <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#navbarUserDropdown" aria-controls="navbarUserDropdown" aria-expanded="false" aria-label="Toggle navigation">
                ...
                <ul class="navbar-nav align-items-center">
                    <li class="nav-item dropdown">
                        <a class="nav-link dropdown-toggle d-flex align-items-center gap-2" href="#" id="userDropdown" role="button" data-bs-toggle="dropdown" aria-expanded="false">
                            <img src="{{ asset('/storage' . (auth()->user()->profile_image ?? 'default-profile.png')) }}" alt="Profile" class="rounded-circle border border-white" style="width: 36px; height: 36px; object-fit: cover;">
                            <span class="text-white fw-semibold">{{ auth()->user()->name }}</span>
                        </a>
                        <ul class="dropdown-menu dropdown-menu-end shadow-lg" aria-labelledby="userDropdown" style="background: radial-gradient(circle, rgba(102,16,242,0.15) 0%, rgba(13,110,253,0.15) 80%); border-radius: 1rem; backdrop-filter: blur(14px);">
                            <li>
                                <a class="dropdown-item d-flex align-items-center gap-2 text-white" href="#" data-bs-toggle="modal" data-bs-target="#profileModal" role="button" tabindex="0" aria-pressed="false" style="background: transparent;">
                                    <i class="fas fa-user me-2 text-light"></i> Profile
                                </a>
                            </li>
                            ...
                            <li>
                                ...
                                <button class="dropdown-item text-warning d-flex align-items-center gap-2" type="button" onclick="event.preventDefault(); document.getElementById('logout-form').submit();" style="background: transparent;">
...
if (pakanButton) {
  pakanButton.addEventListener('click', async () => {
    // Disable & tampilkan spinner
    pakanButton.disabled = true;
    pakanButton.innerHTML = '<span class="spinner-border spinner-border-sm"></span> Memproses...';
    ...
    try {
      ...
      console.error('Error saat memberi pakan:', error);
      showNotif('danger', `<strong>Error!</strong> ${error.message}`);
    } finally {
      // Reset status dan UI setelah 3 detik, apapun hasilnya
      setTimeout(async () => {
        try {
          await db.ref('dashboard/control/pakan').set(false);
        } catch (errReset) {
          console.error('Gagal mereset status pakan:', errReset);
        }
        // Kembalikan tampilan tombol
        pakanButton.disabled = false;
        pakanButton.innerHTML = '<i class="fas fa-broadcast"></i> Beri Pakan';
      }, 3000);
    }
  });
}
