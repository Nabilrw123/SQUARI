...
<div class="main-content">
            ...
            <div class="d-flex align-items-center gap-2 ms-auto">
                <div class="dropdown">
                    <button class="btn btn-link text-dark text-decoration-none dropdown-toggle d-flex align-items-center gap-2" type="button" id="userDropdown" data-bs-toggle="dropdown" aria-expanded="false">
                        <img src="{{ asset('/storage' . (auth()->user()->profile_image ?? 'default-profile.png')) }}" alt="Profile" class="rounded-circle" style="width: 32px; height: 32px; object-fit: cover;">
    ...
    <div class="row g-4 mb-4" style="margin-right: 10px;">
        <div class="col-md-4">
            <div class="stat-card temp">
                <div>
                    <div class="stat-value" id="suhuAir">{{ $sensorData['suhu_ds18b20'] ?? '--' }}°C</div>
        ...
        <div class="col-md-4">
            <div class="stat-card temp">
                <div>
                    <div class="stat-value" id="suhuBME">{{ $sensorData['suhu_bme280'] ?? '--' }}°C</div>
        ...
        <div class="col-md-4">
            <div class="stat-card humidity">
                <div>
                    <div class="stat-value" id="kelembaban">{{ $sensorData['kelembapan'] ?? '--' }}%</div>
        ...
        <div class="col-md-4">
            <div class="stat-card pressure">
                <div>
                    <div class="stat-value" id="tekanan">{{ $sensorData['tekanan'] ?? '--' }} hPa</div>
        ...
        <div class="col-md-4">
            <div class="stat-card water">
                <div>
                    <div class="stat-value" id="levelAir">{{ $sensorData['level_air'] ?? '--' }}%</div>
            ...
            <div class="card">
                <div class="card-header d-flex justify-content-between align-items-center">
                    ...
                    <a href="{{ route('admin.users.index') }}" class="btn btn-primary btn-sm">
                        <i class="fas fa-users me-2"></i> Lihat User
                    </a>
                </div>
                <div class="card-body">
                    <div class="row row-cols-1 row-cols-md-3 g-3">
                        <div class="col">
                            <div class="d-flex align-items-center">
                                <div class="status-indicator {{ $statusData['buzzer'] ?? false ? 'active' : '' }}"></div>
                                <span class="ms-2">Buzzer: {{ $statusData['buzzer'] ?? false ? 'Aktif' : 'Nonaktif' }}</span>
                            </div>
                        </div>
                        <div class="col">
                            <div class="d-flex align-items-center">
                                <div class="status-indicator"></div>
                                <span class="ms-2">Level Air Rendah: {{ $statusData['water_level_low'] ?? '--' }}%</span>
                            </div>
                        </div>
                        <div class="col">
                            <div class="d-flex align-items-center">
                                <div class="status-indicator"></div>
                                <span class="ms-2">Level Air Tinggi: {{ $statusData['water_level_high'] ?? '--' }}%</span>
                            </div>
                        </div>
                        <div class="col">
                            <div class="d-flex align-items-center">
                                <div class="status-indicator"></div>
                                <span class="ms-2">Suhu Udara Rendah: {{ $statusData['suhu_udara_min'] ?? '--' }}°C</span>
                            </div>
                        </div>
                        <div class="col">
                            <div class="d-flex align-items-center">
                                <div class="status-indicator"></div>
                                <span class="ms-2">Suhu Udara Tinggi: {{ $statusData['suhu_udara_max'] ?? '--' }}°C</span>
                            </div>
                        </div>
                        <div class="col">
                            <div class="d-flex align-items-center">
                                <div class="status-indicator"></div>
                                <span class="ms-2">Suhu Air Rendah: {{ $statusData['suhu_air_min'] ?? '--' }}°C</span>
                            </div>
                        </div>
                        <div class="col">
                            <div class="d-flex align-items-center">
                                <div class="status-indicator"></div>
                                <span class="ms-2">Suhu Air Tinggi: {{ $statusData['suhu_air_max'] ?? '--' }}°C</span>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <div class="d-flex justify-content-end mt-4">
        <span class="badge-time" id="lastUpdateDashboard">
            <i class="bi bi-clock"></i> Terakhir update: <span id="dashboardTime">{{ now()->format('H:i:s') }}</span>
        </span>
    </div>
</div>
                ...
                <div class="profile-container">
                    <div class="profile-header text-center">
                        <div class="profile-image-container" id="uploadTrigger">
                            <img src="{{ asset('storage/' . (auth()->user()->profile_image ?? 'default-profile.png')) }}" 
                                 alt="Profile" 
                                 id="profileImage" 
                                 class="profile-image">
                        ...
                        <p class="profile-role">{{ auth()->user()->email }}</p>
                    </div>
...
function updateSensorData(data) {
    ...
    document.getElementById('suhuAir').textContent = `${parseFloat(data.suhu_ds18b20 || 0).toFixed(1)}°C`;
    document.getElementById('suhuBME').textContent = `${parseFloat(data.suhu_bme280 || 0).toFixed(1)}°C`;
    document.getElementById('kelembaban').textContent = `${parseFloat(data.kelembapan || 0).toFixed(1)}%`;
    document.getElementById('tekanan').textContent = `${parseFloat(data.tekanan || 0).toFixed(1)} hPa`;
    document.getElementById('levelAir').textContent = `${parseFloat(data.level_air || 0).toFixed(1)}%`;

    // Update last update time
    const now = new Date();
    document.getElementById('dashboardTime').textContent = now.toLocaleTimeString('id-ID', {
        hour: '2-digit',
        minute: '2-digit',
        second: '2-digit'
    });
...
function updateSystemStatus(data) {
    ...
    // Update temperature and water level status indicators
    const statusIndicators = document.querySelectorAll('.status-indicator');
    const currentSuhuAir = parseFloat(document.getElementById('suhuAir').textContent);
    const currentSuhuUdara = parseFloat(document.getElementById('suhuBME').textContent);
    const currentLevelAir = parseFloat(document.getElementById('levelAir').textContent);

    // Check water level status
    if (currentLevelAir <= data.water_level_low) {
        statusIndicators[1].classList.add('active');
        showAlert('Level air terlalu rendah!', 'danger');
    } else {
        statusIndicators[1].classList.remove('active');
    }
    if (currentLevelAir >= data.water_level_high) {
        statusIndicators[2].classList.add('active');
        showAlert('Level air terlalu tinggi!', 'danger');
    } else {
        statusIndicators[2].classList.remove('active');
    }

    // Check air temperature status
    if (currentSuhuUdara <= data.suhu_udara_min) {
        statusIndicators[3].classList.add('active');
        showAlert('Suhu udara terlalu rendah!', 'warning');
    } else {
        statusIndicators[3].classList.remove('active');
    }
    if (currentSuhuUdara >= data.suhu_udara_max) {
        statusIndicators[4].classList.add('active');
        showAlert('Suhu udara terlalu tinggi!', 'warning');
    } else {
        statusIndicators[4].classList.remove('active');
    }

    // Check water temperature status
    if (currentSuhuAir <= data.suhu_air_min) {
        statusIndicators[5].classList.add('active');
        showAlert('Suhu air terlalu rendah!', 'warning');
    } else {
        statusIndicators[5].classList.remove('active');
    }
    if (currentSuhuAir >= data.suhu_air_max) {
        statusIndicators[6].classList.add('active');
        showAlert('Suhu air terlalu tinggi!', 'warning');
    } else {
        statusIndicators[6].classList.remove('active');
    }
}
...
function refreshData() {
    // Get sensor data
    db.ref('dashboard/sensor').once('value')
        .then((snapshot) => {
    ...
    // Get system status
    db.ref('dashboard/status').once('value')
        .then((snapshot) => {
            const data = snapshot.val() || {};
...
function updateClock() {
    const now = new Date();
    document.getElementById('clockTimeModern').textContent = now.toLocaleTimeString();
    document.getElementById('clockDateModern').textContent = now.toLocaleDateString('id-ID', {
        weekday: 'long',
        year: 'numeric',
        month: 'long',
        day: 'numeric'
...
// Pakan button functionality
const pakanButton = document.getElementById('pakanButtonDashboard');
if (pakanButton) {
    pakanButton.addEventListener('click', async function() {
        try {
            this.disabled = true;
...
function processAlertQueue() {
    ...
    // Hapus alert setelah 3 detik, lalu tampilkan alert berikutnya
    setTimeout(() => {
